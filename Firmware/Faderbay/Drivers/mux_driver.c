...
fb_err_t MuxDriver_SelectChannel(uint8_t channel) {
    if (!s_initialized) {
        LOGE(TAG, "SelectChannel: not initialized");
        return FB_ERR_NOT_READY;
    }
    if (channel >= s_config.num_channels) {
        LOGE(TAG, "SelectChannel: out of range");
        return FB_ERR_INVALID_PARAM;
    }
